...
qPT = np.load(f'../../Data/Omega/Pq/Pq_PA_{seed}_reference.npy')
temps_PT =  np.load(f'../../Data/Omega/Pq/temperatures_PA_{seed}_reference.npy')
...
indices_reference = [np.where(temps_PT == float(value))[0].item() for value in tempPT]
print(f"PA_long, {len(temps_PT)}, {indices_reference}")
print(temps_PT[indices_reference])
...
for n in range(4):
    for i, alg in enumerate(algorithms):
        ...
        counts_pt, bins, _ = ax.hist(
            np.concatenate((qPT[indices_reference[n]],-qPT[indices_reference[n]])),
            bins=50,
            alpha=0.5,
            density=True,
            color="darkgrey",
