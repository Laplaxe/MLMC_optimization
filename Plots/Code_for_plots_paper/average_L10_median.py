...
# ALSO: list seeds whose intensive GS energy is higher (worse) than -1.6980
threshold = -1.6980
bad_seeds = []
for fname, e in gs_values.items():
    seed = int(fname.split('_')[1].split('.')[0])
    if e > threshold:
        bad_seeds.append((seed, e))
bad_seeds.sort(key=lambda t: t[1], reverse=True)  # worst first

if bad_seeds:
    print(f"Seeds with GS energy > {threshold:.4f} (intensive):")
    for seed, e in bad_seeds:
        print(f"  seed {seed}: e = {e:.6f}")
else:
    print(f"No seeds exceed GS energy threshold {threshold:.4f} (intensive).")

# ─── 2) GATHER per‐seed succ_prob & avg_time ──────────────────────────────────
def gather_stats(dirpath, cols):
    ...
    for fname in filenames:
        ...
        GS = gs_values[fname]  # success check uses intensive equality
        grouped = (
            df
            .groupby('nTemps')
            .agg(
...
def plot_imputed_band(stats, imp, color, alg_name):
    ...
    plt.plot(x, mid, linestyle='-', color=color, lw=3, label=alg_name, zorder=4)  # median
...
plt.savefig("../Figures_paper/avg_with_runs_success_vs_time_L10.png", dpi=300)
plt.savefig("../Figures_paper/avg_with_runs_success_vs_time_L10.pdf")
